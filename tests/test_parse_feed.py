...
def test_parse_feed_extracts_trip_update():
    ...
    trip_updates = [r for r in results if r.entity_type == "trip_update"]
...
def test_parse_feed_extracts_vehicle_position():
    ...
    positions = [r for r in results if r.entity_type == "vehicle_position"]
    assert len(positions) == 1
    assert positions[0].vehicle_lat == 40.730610
    assert positions[0].vehicle_lon == -73.935242
